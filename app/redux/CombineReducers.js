...
const userFollowingDefault = {
  following: [],
  total: 0,
  loading: false,
  refreshing: false
};
function userFollowing(state = userFollowingDefault, action) {
  switch (action.type) {
...
const userClipsDefault = {
  clips: [],
  cursor: '',
  loading: false,
  refreshing: false
};
function userClips(state = userClipsDefault, action) {
  switch (action.type) {
    ...
    case USER_CLIPS_RESPONSE:
      return Object.assign({}, state, {
        clips: state.clips.concat(action.clips),
...
const userVideoActionsDefault = {
  videos: [],
  total: 0,
  loading: false,
  refreshing: false
};
// Wrap Reducer in a function so it can be scoped and reused across views
function userVideoActions(scope = 'generic') {
  return function userVideos(state = userVideoActionsDefault, action) {
...
function topClips(state = topClipsDefault, action) {
  switch (action.type) {
...
function userStuff(state = userStuffDefault, action) {
  switch (action.type) {
    case SET_USERS_INFO:
      return Object.assign({}, state, action, {
        loadingUserInfo: false
    ...
    case CHANNEL_FOLLOWERS_RESPONSE:
      return Object.assign({}, state, action, {
        loadingFollowers: false,
        follows: state.follows.concat(action.follows)
      });
    default:
      return state;
  }
...
function bookmarks(state = bookmarksDefault, action) {
